...
// Flatten MongoDB participants[] array into flat row keys DataTable expects
function flattenRow(reg: any): Record<string, any> {
...
const BGMILastSquad: React.FC = () => {
    const [data, setData]       = useState<any[]>([]);
    const [loading, setLoading] = useState(true);
    const [error, setError]     = useState<string | null>(null);

    useEffect(() => {
        fetch(`${API_BASE}/registrations/bgmi-lss`)
            .then((res) => res.json())
            .then((json) => {
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0f] text-white">
            ...
            <div className="max-w-screen-xl mx-auto px-6 py-10 space-y-8">
                <div>
                    <h1 className="text-3xl font-bold text-white mb-1" style={{ fontFamily: '"Ikaros", sans-serif' }}>🪖 BGMI – Last Squad Standing</h1>
                    <p className="text-white/40" style={{ fontFamily: '"Proza Libre", sans-serif' }}>GROUP · Team Size 4 · Table: BGMI_Last_Squad_Standing</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <StatsCard title="Total Registrations" value={loading ? "…" : data.length} icon="👥" color="bg-green-600"   />
                    <StatsCard title="Total Teams"         value={loading ? "…" : teams}       icon="🏆" color="bg-teal-500"    />
                    <StatsCard title="Departments"         value={loading ? "…" : depts}       icon="🏫" color="bg-emerald-500" />
                </div>
