...
const COLUMNS = [
    ...
    { key: "Full Name-5", label: "Member 5" },
    { key: "USN-5", label: "USN 5" },
];

const ValorantBattle: React.FC = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            if (!supabase) return;
            const { data: rows, error } = await supabase.from("VALORANT_Tournament").select("*");
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0f] text-white">
            ...
            <div className="max-w-screen-xl mx-auto px-6 py-10 space-y-8">
                <div>
                    <h1 className="text-3xl font-bold text-white mb-1" style={{ fontFamily: '"Ikaros", sans-serif' }}>🎮 Valorant Battle</h1>
                    <p className="text-white/40" style={{ fontFamily: '"Proza Libre", sans-serif' }}>GROUP · Team Size 5 · Table: VALORANT_Tournament</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <StatsCard title="Total Registrations" value={loading ? "…" : data.length} icon="👥" color="bg-red-600" />
                    <StatsCard title="Total Teams" value={loading ? "…" : teams} icon="🏆" color="bg-orange-500" />
                    <StatsCard title="Departments" value={loading ? "…" : depts} icon="🏫" color="bg-yellow-500" />
                </div>
                {loading ? (
