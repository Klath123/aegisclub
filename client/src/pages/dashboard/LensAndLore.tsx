...
const LensAndLore: React.FC = () => {
    ...
    useEffect(() => {
        async function fetchData() {
            if (!supabase) return;
            // Table name has a space before the underscore as set in RegisterForm
            const { data: rows, error } = await supabase.from("Lens_& _Lore").select("*");
    ...
    const pairs = data.filter((r) => r["Full Name-2"]).length;
